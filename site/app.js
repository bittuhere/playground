(() => {
  ...
  async function loadData() {
    try {
      const [gamesData, profileData] = await Promise.all([getJSON("/api/games"), getJSON("/api/profile")]);
  ...
  async function saveProfileData(data, closeAfter = true) {
    try {
      const updated = await getJSON("/api/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
  ...
  async function sendFriendRequest(username) {
    try {
      await getJSON("/api/friends/request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username }) });
  ...
  document.addEventListener("submit", (event) => {
    ...
    if (event.target.id === "feedbackForm") {
      ...
      getJSON("/api/feedback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message }) }).then(() => { closeModal(); showToast("Thanks — feedback saved."); }).catch(() => { closeModal(); showToast("Feedback saved for this session."); });
    }
